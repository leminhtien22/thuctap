...
<div class="bg-[#1c1c1c] text-white py-16 px-4 sm:px-8 font-poppins min-h-screen">
    <div class="max-w-7xl mx-auto">
        <x-ui.breadcrumb :is-admin="0" is-dark :breadcrumbs="[['url' => 'client.shop', 'label' => 'Shop']]" />
        ...
        <!-- Search bar -->
        <form action="{{ route('client.shop') }}" method="GET" class="mb-14">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
            @forelse ($data as $item)
                <div class="relative bg-[#2f2f2f] rounded-2xl overflow-hidden shadow-md hover:shadow-yellow-400/40 transition duration-300">
                    ...
                    <a href="{{ route('client.shop.details', $item->id) }}">
            ...
            @empty
                <div class="col-span-3">
                    <x-ui.alert type="warning">
                        {{ $searching ? 'No shop items found with keyword "' . $searching . '"' : 'No shop items are currently scheduled' }}
                    </x-ui.alert>
                </div>
            @endforelse
        </div>
