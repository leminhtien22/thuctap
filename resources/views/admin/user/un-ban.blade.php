...
@section('title')
    {{ __('Mở khoá tài khoản người dùng') }}
@endsection

@section('content')
    <x-ui.breadcrumb :breadcrumbs="[
        ['url' => 'admin.user', 'label' => 'Người dùng'],
        ['url' => 'admin.user.create', 'label' => 'Mở khóa tài khoản'],
    ]" />

    <form class="space-y-4 md:space-y-6 mt-8" action="{{ route('admin.user.unBan', $user->id) }}" method="POST">
        @csrf
